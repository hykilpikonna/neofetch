...
def print_progressbar(total: int, i: int, length: int | None = None, theme: Theme = EMOJI_THEME, unit=''):
    if not length:
        length = hyfetch.constants.TERM_WIDTH
    i += 1
...
def download_pbar(url: str, path: Path):
    def hook(b: int, bsize: int, tsize: int):
        print_progressbar(tsize // 1024 // 1024, b *
                          bsize // 1024 // 1024, unit=' MB')
...
if __name__ == '__main__':
    ...
    #     print()
    download_pbar(
        'https://github.com/git-for-windows/git/releases/download/v2.37.2.windows.2/MinGit-2.37.2.2-busybox-64-bit.zip', Path('Downloads'))
